...
const SearchListDisplay = () => {
    ...
    const { id } = params;
    const typesOfLists: string[] = ["Search", "SearchFilter"];
    ...
    const fetchData = async (page: number = 1) => {
        ...
        try {
            let response;
            if (isFiltered) {
                console.log("okay tat")
                    setFilteredState(true)
                    response = await anime_service.getFilteredSearchList(currentFilterId,currentTypeOfItems,currentStateOfItems,searchTerm,page);
            } else {
                console.log("yes we are")
                response = await anime_service.getSearchResultByTerm(page,searchQuery);
            }
    ...
    const getThePageTitle = () => {
        ...
        return `${searchQuery} | ${genre}${type ? ` | ${type}` : ""}${state ? ` | ${state}` : ""}`;
    };
        ...
        <div className="bg-gray-900 rounded-lg p-6">
            ...
            <div className="mb-8">
                <div className="flex items-center justify-between">
                    ...
                    {isFiltered && (
                        <Link 
                            href={`/search/Search?q=${searchQuery}`}
