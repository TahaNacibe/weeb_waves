...
const MovieListDisplay = () => {
    ...
    const { id } = params;
    const typesOfLists: string[] = ["Movies", "Filter"];
    ...
    const fetchData = async (page: number = 1) => {
        ...
        try {
            ...
            switch (decodedId) {
                case typesOfLists[0]:
                    response = await anime_service.getMoviesListByNewts(page);
                    break;
                case typesOfLists[1]:
                    setFilteredState(true)
                    response = await anime_service.getFilteredMovieList(currentFilterId,currentStateOfItems,page);
                    break;
                default:
                    return;
            }
    ...
    const getThePageTitle = () => {
        ...
        return `${genre} | Movie ${state ? ` | ${state}` : ""}`;
    };
    ...
    const displayData = () => {
        if (!animeList) {
            ...
            if (animeList.length == 0) {
                {/* empty case */ }
                return (<ErrorWidget type={1} title='No Movies?' message={isFiltered? `Seems Theres No Movies With Tags you choose`:`Something went wrong here`} />)
            } else {
                {/* normal case */ }
                return (<div>
                    {/* Grid Layout */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 -m-6">
                        {animeList?.map((elem, index) => (
                            ...
                            >
                                <AnimeWidget 
                                    anime={elem} 
                                    isRanked={decodedId === "Movies" || isFiltered} 
                                />
                            </div>
                        ))}
                    </div>
...
export default MovieListDisplayPage;